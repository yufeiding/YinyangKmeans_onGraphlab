/*  
 * Note by Yufei Ding: The only thing differs from orignal kmeans code in Graphlab is the center initialization.
 * Instead of using kmeans++, the initial centers are set to the first k points in the input data set.
 *
...
void kmeans_rigid_initialization(graph_type::vertex_type& v)
{
  //  size_t t;
  if(v.id()<NUM_CLUSTERS){
    // t= v.id();
    //std::cout <<"v.id()"<<t<<std::endl;
    CLUSTERS[v.id()].center = v.data().point;
  }
}
